	...
	public void execute(@MethodParam(Execute.CONTEXT) IBruiContext context) {
		...
		if ("采购".equals(catagory)) {
			...
			if (InputDialog.OK == id.open()) {
				...
				Document doc = new SqlQuery("ecology").sql("select * from V_PMS_SO_PR_PO_IN where so_num='" + so_num + "'").first();
				if (doc != null) {
					String prt_desc = doc.get("SO_PRT_DESC").toString();
					String prt_num = doc.get("SO_PRT_NUM").toString();
