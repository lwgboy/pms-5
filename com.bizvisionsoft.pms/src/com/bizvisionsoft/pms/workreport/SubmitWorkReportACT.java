...
public class SubmitWorkReportACT {
	...
	public void execute(@MethodParam(Execute.CONTEXT) IBruiContext context) {
		...
		boolean ok = br.confirm("提交" + input.getType(),
				"请确认提交报告：" + input.getLabel() + "。\n系统将记录现在时刻为报告提交时间，提交后该报告将无法进行修改。");
		if (!ok) {
			return;
		}
		List<Result> result = Services.get(WorkReportService.class).submitWorkReport(Arrays.asList(input.get_id()), br.getDomain());
		if (result.isEmpty()) {
			Layer.message("报告已提交");
