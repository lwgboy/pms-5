...
public class BudgetCBS extends CBSGrid {
	...
	static class ExtendLabel extends ColumnLabelProvider {
		...
		public static ColumnLabelProvider newInstance(Function<CBSItem, String> func, Function<CBSItem, Color> colorFunc) {
			ExtendLabel p = new ExtendLabel();
			p.func = func;
	...
	public void addCBSItem(CBSItem parentCBSItem, CBSItem cbsItemData) {
		try {
			CBSItem child = Services.get(CBSService.class).insertCBSItem(cbsItemData, br.getDomain());
	...
	public void deleteCBSItem(CBSItem cbsItem) {
		...
		Services.get(CBSService.class).delete(cbsItem.get_id(), br.getDomain());
	...
	public void updateCBSPeriodBudget(CBSItem cbsItem, CBSPeriod periodData) {
		...
		Services.get(CBSService.class).updateCBSPeriodBudget(periodData, br.getDomain());
		CBSItem newCbsItem = Services.get(CBSService.class).get(((CBSItem) cbsItem).get_id(), br.getDomain());
	...
	protected String getMonthlyAmountText(Object element, String name) {
		return Formatter.getMoneyFormatString(((CBSItem) element).getPeriodBudget(name));
	}
	...
	protected void updateCBSItemPeriodBudgetInput(CBSItem item, String name, Object input) throws Exception {
		...
		///////////////////////////////////////
		// 避免在没有修改的时候调用服务端程序
		double oldAmount = item.getPeriodBudget(name);
	...
	// vcol.setLabelProvider(ExtendLabel.newInstance(t ->
	// getNumberText(t.getTotalEstimation(), "0.00"),
	...
	// vcol = createColumn(grp, c);
	// vcol.setLabelProvider(ExtendLabel.newInstance(t ->
	// getNumberText(getBudgetPerEpisode(t), "0.00"),
	// t -> BruiColors.getColor(BruiColor.Grey_50)));
	//
	// // 费用/分钟
	// c = new Column();
	...
	// vcol = createColumn(grp, c);
	// vcol.setLabelProvider(ExtendLabel.newInstance(t ->
	// getNumberText(getBudgetPerMinite(t), "0.00"),
	// t -> BruiColors.getColor(BruiColor.Grey_50)));
	//
	// // 总费用来源
	// c = new Column();
	...
	// ExtendLabel.newInstance(t -> getSourceText(t), t ->
	// BruiColors.getColor(BruiColor.Grey_50)));
	//
	// // 数量
	// c = new Column();
	// c.setText("数量");
	...
	// c.setSummary(true);
	// vcol = createColumn(grp, c);
	// vcol.setLabelProvider(ExtendLabel.newInstance(t -> getNumberText(t.getQty(),
	// "0")));
	// vcol.setEditingSupport(new EditingSupport(viewer) {
	//
	// @Override
	...
	// return;
	// item.setQty(v);
	// BasicDBObject fu = new FilterAndUpdate().filter(new BasicDBObject("_id",
	// item.get_id()))
