	...
	public void execute(@MethodParam(Execute.ROOT_CONTEXT_INPUT_OBJECT) IWBSScope rootInput) {
		...
		if (workspace != null) {
			...
			if (checkoutUserId == null || "".equals(checkoutUserId) || brui.getCurrentUserId().equals(checkoutUserId)) {
				...
				Result result = Services.get(WorkSpaceService.class).checkout(workspace, brui.getCurrentUserId(), false);
				if (Result.CODE_WORK_SUCCESS == result.code) {
					...
					if (MessageDialog.openConfirm(brui.getCurrentShell(), "提示",
						...
						result = Services.get(WorkSpaceService.class).checkout(workspace, brui.getCurrentUserId(), true);
						if (Result.CODE_WORK_SUCCESS == result.code) {
							brui.switchContent("项目甘特图（编辑）", workspace);
						}
					}
				...
				User checkoutUser = Services.get(UserService.class).get(checkoutUserId);
				brui.error("提示", checkoutUser + "已经检出计划，在该计划检入或取消检出以前，不能进行编辑。");
			}
		}
	}
