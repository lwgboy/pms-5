...
public class WorkSpaceServiceImpl extends BasicServiceImpl implements WorkSpaceService {
	...
	public List<Result> schedulePlanCheck(Workspace workspace, Boolean checkManageItem) {
		...
		Document systemSetting = getScopeSetting(project.get_id(), CHECKIN_SETTING_NAME);
		...
		// 获取早于项目计划开始的设置，默认为禁止
		setting = getSettingValue(systemSetting, CHECKIN_SETTING_FIELD_PROJECT_START, CHECKIN_SETTING_VALUE_REQUIREMENT);

		Date _start = doc.getDate("start");
		if (CHECKIN_SETTING_VALUE_AUTO.equals(setting) && planStart.after(_start)) {
			if (work_id != null) {
				c("work").updateOne(new BasicDBObject("_id", work_id), new BasicDBObject("$set", new BasicDBObject("planStart", _start)));
			} else {
				c("project").updateOne(new BasicDBObject("_id", workspace.getProject_id()),
						new BasicDBObject("$set", new BasicDBObject("planStart", _start)));
			}
		} else if (CHECKIN_SETTING_VALUE_REQUIREMENT.equals(setting) && planStart.after(_start)) {
			results.add(Result.error("工作最早计划开始时间早于项目计划开始时间。"));
		} else if (CHECKIN_SETTING_VALUE_WARNING.equals(setting) && planStart.after(_start)) {
			results.add(Result.question("工作最早计划开始时间早于项目计划开始时间。"));
		}
		// 获取晚于项目计划完成的设置，默认为禁止
		setting = getSettingValue(systemSetting, CHECKIN_SETTING_FIELD_PROJECT_FINISH, CHECKIN_SETTING_VALUE_REQUIREMENT);

		Date _finish = doc.getDate("finish");
		if (CHECKIN_SETTING_VALUE_AUTO.equals(setting) && planFinish.before(_finish)) {
			if (work_id != null) {
				c("work").updateOne(new BasicDBObject("_id", work_id), new BasicDBObject("$set", new BasicDBObject("planFinish", _finish)));
			} else {
				c("project").updateOne(new BasicDBObject("_id", workspace.getProject_id()),
						new BasicDBObject("$set", new BasicDBObject("planFinish", _finish)));
			}
		} else if (CHECKIN_SETTING_VALUE_REQUIREMENT.equals(setting) && planFinish.before(_finish)) {
			results.add(Result.error("工作最完计划完成时间晚于项目计划完成时间。"));
		} else if (CHECKIN_SETTING_VALUE_WARNING.equals(setting) && planFinish.before(_finish)) {
