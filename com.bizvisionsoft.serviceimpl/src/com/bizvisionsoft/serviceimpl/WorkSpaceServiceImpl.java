...
public class WorkSpaceServiceImpl extends BasicServiceImpl implements WorkSpaceService {
	 ...
	 * @return
	 */
	private ArrayList<Result> checkAssignment(String defaultSetting, Map<String, Object> mLvlSetting, ObjectId space_id, boolean onlyCheck,
			String domain) {
		// 获取没有指定负责人和指派者的工作
		ArrayList<Result> results = new ArrayList<>();
	...
	public Result checkin(Workspace workspace, String domain) {
		...
		List<ObjectId> workIds = c(Work.class, domain)
		...
		if (!insertIds.isEmpty()) {
			...
			c("workspace", domain).find(new BasicDBObject("_id", new BasicDBObject("$in", insertIds))).forEach((Document d) -> {
				insertDoc.add(d);
				Optional.ofNullable(d.getObjectId("parent_id")).ifPresent(parentIds::add);
			});
			...
			c("work", domain).updateMany(new BasicDBObject("_id", new BasicDBObject("$in", parentIds)),
			...
			if (Arrays.asList(ProjectStatus.Closing, ProjectStatus.Processing).contains(projectStatus)) {
				...
				if (project.isStageEnable()) {
					...
					c("work", domain).aggregate(
							Domain.getJQ(domain, "查询-工作-阶段需下达的工作计划").set("project_id", project_id).set("match", new Document()).array())
	...
	private void updateWorkPlanWorks(ObjectId work_id, String domain) {
		if (work_id != null) {
			...
			double planWorks = Optional.ofNullable(c("resourcePlan", domain).aggregate(pipeline).first())
