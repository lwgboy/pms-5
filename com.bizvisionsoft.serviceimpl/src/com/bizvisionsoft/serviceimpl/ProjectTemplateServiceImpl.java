...
public class ProjectTemplateServiceImpl extends BasicServiceImpl implements ProjectTemplateService {
	...
	public List<WorkInTemplate> listWorks(ObjectId template_id, String domain) {
		return c(WorkInTemplate.class, domain).find(new BasicDBObject("template_id", template_id))
	...
	public int nextOBSSeq(BasicDBObject condition, String domain) {
		Document doc = c("obsInTemplate", domain).find(condition).sort(new BasicDBObject("seq", -1))
	...
	public List<WorkInTemplate> listWBSRoot(ObjectId template_id, String domain) {
		List<? extends Bson> pipeline = Domain.getJQ(domain, "查询-模板工作-WBS根").set("template_id", template_id)
				.set("parent_id", null).array();
	...
	public long deleteHumanResourcePlan(ObjectId work_id, String resId, String domain) {
		DeleteResult dr = c("resourcePlanInTemplate", domain)
	...
	public List<ResourcePlanInTemplate> listResourcePlan(ObjectId _id, String domain) {
		return c(ResourcePlanInTemplate.class, domain)
				.aggregate(Domain.getJQ(domain, "查询-模板资源").set("work_id", _id).array())
				.into(new ArrayList<ResourcePlanInTemplate>());
	}

	@Override
	public long delete(ObjectId _id, String domain) {
		ArrayList<ObjectId> ids = c("workInTemplate", domain)
				.distinct("_id", new Document("template_id", _id), ObjectId.class).into(new ArrayList<>());
	...
	public void setEnabled(ObjectId _id, boolean enabled, String domain) {
		c("projectTemplate", domain).updateOne(new Document("_id", _id),
	...
	public void useTemplate(ObjectId template_id, ObjectId project_id, String user, String domain) {
		...
		List<ObjectId> workIds = c("work", domain)
				.distinct("_id", new Document("project_id", project_id), ObjectId.class).into(new ArrayList<>());
		// 2.清除关联的resourcePlan
		...
		// 9. 清除obs
		c("obs", domain)
				.deleteMany(new Document("$or", Arrays.asList(new Document("scope_id", new Document("$in", workIds)),
						new Document("scopeRoot", false).append("scope_id", project_id))));
		// 10. 清除cbs
		List<ObjectId> cbsIds = c("cbs", domain).distinct("_id",
				new Document("$or",
		...
		// 11. 清除folder
		c("folder", domain).deleteMany(
				new Document("project_id", project_id).append("isflderroot", new BasicDBObject("$ne", true)));
		// 12. 清楚document
		// TODO

		// 13.清除项目设置
		c("setting", domain).deleteMany(
				new Document("name", Pattern.compile("@" + project_id.toString(), Pattern.CASE_INSENSITIVE)));
		...
		folderIdMap.put(null, project.getObjectId("folder_id"));// 将项目根文件夹添加到对应表中
		c("folderInTemplate", domain).find(new Document("template_id", template_id)).sort(new Document("parent_id", 1))
				.forEach((Document doc) -> {
					...
					Object desc = doc.get("name");
					doc.append("_id", folder_id).append("project_id", project_id)
							.append("parent_id", folderIdMap.get(doc.getObjectId("parent_id"))).append("desc", desc)
							.append("root_id", project.get("container_id"));
		...
		c("workPackage", domain).find(new Document("work_id", new Document("$in", workIdMap.keySet())))
				.forEach((Document d) -> {
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// 创建工作的资源计划
		c("resourcePlanInTemplate", domain).find(new Document("work_id", new Document("$in", workIdMap.keySet())))
				.forEach((Document d) -> {
		...
		c("obsInTemplate", domain).find(new Document("scope_id", template_id)).sort(new Document("_id", 1))
				.forEach((Document doc) -> {
					ObjectId parent_id = doc.getObjectId("parent_id");
					if (parent_id == null) {
		...
		// 添加项目设置
		List<Document> list = PROJECT_SETTING_NAMES.stream().map(name -> {
			Document setting = Optional.ofNullable(getSystemSetting(name + "@" + template_id.toString(), domain))
					.orElse(new Document());
	...
	public OBSModule insertOBSModule(OBSModule obsModule, String domain) {
		...
		insert(OBSInTemplate.getInstanceTeam(obsModule.get_id(), null, obsModule.getName(), obsModule.getId(),
				obsModule.getName(), true, domain), domain);// 插入根节点记录
		return obsModule;
	}
	...
	public void useOBSModule(ObjectId obsModule_id, ObjectId project_id, String domain) {
		...
		// 项目的OBS_ID
		ObjectId obs_id = c("project", domain).distinct("obs_id", new Document("_id", project_id), ObjectId.class)
				.first();

		// 创建组织结构
		c("obsInTemplate", domain).find(new Document("scope_id", obsModule_id)).sort(new Document("_id", 1))
				.forEach((Document doc) -> {
					// 建立项目团队上下级关系
					ObjectId parent_id = doc.getObjectId("parent_id");
					if (parent_id == null) {
	...
	public long countOBSModuleSelector(BasicDBObject filter, ObjectId project_id, String domain) {
		// 获取当前项目的eps_id
		ObjectId eps_id = c("project", domain).distinct("eps_id", new Document("_id", project_id), ObjectId.class)
				.first();
