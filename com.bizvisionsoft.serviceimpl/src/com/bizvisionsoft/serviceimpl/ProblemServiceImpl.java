...
public class ProblemServiceImpl extends BasicServiceImpl implements ProblemService {
	...
	@Override
	public boolean selectProblemsCard(ObjectId _id, String userId, String domain) {
		List<Bson> pipeline = new ArrayList<>(); 
		pipeline.addAll(Domain.getJQ(domain, "追加-问题查询用户与权限").set("userId", userId).set("_id", _id).array());
		ArrayList<Document> result = c("problem", domain).aggregate(pipeline).into(new ArrayList<>());
		if(result.size()==0) {
			return true;
		}
		return false;
