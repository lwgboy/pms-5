...
public class ProductServiceImpl extends BasicServiceImpl implements ProductService {
	...
	public List<ProductBenchmark> projectProductBenchmarking(ObjectId project_id, String domain) {
		List<Bson> pipeline = Domain.getJQ(domain, "图表-销售-产品对标").set("project_id", project_id).array();
...
//	 <产品销售额占比> <对标产品销售额占比> <系列其他产品销售额占比>
		...
		Document data = c("product",domain).aggregate(Domain.getJQ(domain, "查询-销售-产品对标").set("product_id", product_id).array()).first();
		JQ jq = Domain.getJQ(domain, "图表-销售-产品对标");
