...
public class ProjectServiceImpl extends BasicServiceImpl implements ProjectService {
	...
	public List<Docu> listDocument(ObjectId folder_id) {
		...
		pipe.add(new Document("$match",new Document("folder_id", folder_id)));
		appendUserInfo(pipe, "createBy", "createByInfo");
		return c(Docu.class).aggregate(pipe).into(new ArrayList<>());
	}
